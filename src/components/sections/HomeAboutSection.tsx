...
export function HomeAboutSection({ content, 'data-section-key': sectionKey, 'data-section-id': sectionId }: HomeAboutSectionProps = {}) {
  const { theme } = useTheme();
  const contentRef = useScrollReveal();

  const title = content?.title || "The Master Behind the Menu";
          ...
          >
            ...
            {paragraphs.map((paragraph, index) => (
              <p
                key={index}
                className="transition-colors duration-300"
                data-section-component-key={`paragraphs.${index}`}
